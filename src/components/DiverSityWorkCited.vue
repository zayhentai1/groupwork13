<template>
	<div id="diversity-work-cited-wrapper"></div>
</template>
...
async function getMultipleLineChart() {
	// 1. Access data
	const dataset = await d3.csv('./diversity_work_cited.csv')
	...
	// 3. Draw canvas
	const wrapper = d3
		.select('#diversity-work-cited-wrapper')
		.append('svg')
	...
	const yScale = d3
		.scaleLinear()
		.domain([
			d3.min(dataset, (row) => d3.min(group, (key) => +row[key])),
			d3.max(dataset, (row) => d3.max(group, (key) => +row[key])),
		])
	...
	const lines = bounds
		...
		.enter()
		.append('path')
		.attr('class', 'diversity-work-cited-line')
		.attr('id', (d, i) => 'diversity-work-cited-line' + i)
		.attr('d', (d) =>
			lineGenerator(
	...
	const legend = bounds
		...
		.on('click', function (event, d) {
			const line = d3.select('#diversity-work-cited-line' + group.indexOf(d))
	...
	const title = bounds
		...
		.attr('y', -10)
		.text('Diversity of Work Cited, 1980-2010 ')
		.attr('fill', 'black')
