<template>
	<div id="cited-selft-wrapper"></div>
</template>
...
async function getMultipleLineChart() {
	// 1. Access data
	const dataset = await d3.csv('./cited_self.csv')
	...
	const lines = bounds
		...
		.enter()
		.append('path')
		.attr('class', 'cited-self-line')
		.attr('id', (d, i) => 'cited-self-line' + i)
		.attr('d', (d) =>
			lineGenerator(
	...
	const legend = bounds
		...
		.on('click', function (event, d) {
			const line = d3.select('#cited-self-line' + group.indexOf(d))
	...
	const title = bounds
		...
		.attr('y', -10)
		.text('Mean self-citations, 1980-2010 ')
		.attr('fill', 'black')
	...
	const yAxisLabel = yAxis
		...
		.style('font-size', '1.4em')
		.style('font-weight', 'bold')
		.text('Mean self-citations per patent')
