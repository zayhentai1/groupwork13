<template>
	<div id="cited-age_mean-wrapper"></div>
</template>
...
async function getMultipleLineChart() {
	// 1. Access data
	const dataset = await d3.csv('./cited_age_mean.csv')
	...
	const lines = bounds
		...
		.enter()
		.append('path')
		.attr('class', 'cited_age-line')
		.attr('id', (d, i) => 'cited_age-line' + i)
		.attr('d', (d) =>
			lineGenerator(
	...
	const legend = bounds
		...
		.on('click', function (event, d) {
			const line = d3.select('#cited_age-line' + group.indexOf(d))
	...
	const title = bounds
		...
		.attr('y', -10)
		.text('Mean Age of Work Cited, 1980-2010 ')
		.attr('fill', 'black')
