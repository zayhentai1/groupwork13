<template>
	<div id="language-unique-words-wrapper"></div>
</template>
...
async function getMultipleLineChart() {
	// 1. Access data
	const dataset = await d3.csv('./unique_words.csv')
	...
	// 3. Draw canvas
	const wrapper = d3
		.select('#language-unique-words-wrapper')
		.append('svg')
	...
	const lines = bounds
		...
		.enter()
		.append('path')
		.attr('class', 'unique-words-line')
		.attr('id', (d, i) => 'unique-words-line' + i)
		.attr('d', (d) =>
			lineGenerator(
	...
	const legend = bounds
		...
		.on('click', function (event, d) {
			const line = d3.select('#unique-words-line' + group.indexOf(d))
	...
	const title = bounds
		...
		.attr('y', -10)
		.text('Diversity of Language Used in Technology (Unique), 1980-2010 ')
		.attr('fill', 'black')
